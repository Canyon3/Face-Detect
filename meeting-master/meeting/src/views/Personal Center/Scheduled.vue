  ...
  <div>
    ...
    <div style="padding: 10px 0">
      <el-input
        ...
        clearable
      ></el-input>
      <el-button style="margin-left: 3px" type="primary" @click="load"
        >搜索</el-button
      >
      <el-button style="margin-left: 3px" type="warning" @click="reset"
        >重置</el-button
      >
    </div>
      ...
      <el-table-column fixed="right" label="操作" width="480">
        <template slot-scope="scope">
          <el-button
            type="danger"
            size="small"
            @click="cancelmeeting(scope.row.meetingid)"
            >撤销会议<i class="el-icon-circle-close"></i
          ></el-button>
          <el-button
            type="primary"
            size="small"
            @click="participants(scope.row.meetingid)"
            >查看考勤信息<i class="el-icon-view"></i
          ></el-button>
          <el-button
            type="success"
            size="small"
            @click="addMeeting(scope.row.meetingid)"
            >添加参会员工<i class="el-icon-circle-plus-outline"></i
          ></el-button>
    ...
    >
      <div style="margin-bottom: 10px">
        参加会议总人数：{{ totalNumber }} 人<el-divider
          direction="vertical"
        ></el-divider>
        已签到人数：{{ totalNumber - signedNumber }} 人<el-divider
          direction="vertical"
        ></el-divider>
        未签到人数：{{ signedNumber }} 人
        <el-divider direction="vertical"></el-divider>
        <el-button
          type="primary"
          @click="exp(meetingid)"
          style="margin-left: 10px"
          >导出考勤信息<i class="el-icon-download"></i
        ></el-button>
      </div>
      <el-table
        :data="employeeData"
        ...
        <el-table-column prop="status" label="签到状态" width="100px">
          <template slot-scope="scope">
            <el-tag
              size="small"
              v-if="
                scope.row.status === 0 && nowDate >= scope.row.signinendtime
              "
              type="danger"
              >未签到
            </el-tag>
            <el-tag
              size="small"
              v-if="
                scope.row.status === 0 && nowDate <= scope.row.signinstarttime
              "
              type="info"
            >
              签到未开始
            ...
            <el-tag
              ...
              v-if="
                scope.row.status === 0 &&
                nowDate <= scope.row.signinendtime &&
                nowDate >= scope.row.signinstarttime
              "
            >
              正在签到
            </el-tag>
        ...
        <el-table-column fixed="right" label="操作">
          <template slot-scope="scope">
            <el-button
              type="success"
              size="small"
              @click="signature(scope.row.meetingid, scope.row.employeeid)"
              v-if="
                scope.row.status === 0 && nowDate >= scope.row.signinendtime
              "
              >补签<i class="el-icon-check"></i
            ></el-button>
            <el-button
              type="danger"
              size="small"
              @click="DeleteEmp(scope.row.employeeid, scope.row.meetingid)"
    ...
    >
      <el-form
        ...
        :model="formRevoke"
        ref="formRevoke"
        :rules="formRevokeFormRules"
      >
        <el-form-item label="会议名称" prop="meetingname">
          <el-input
            v-model="formRevoke.meetingname"
            autocomplete="off"
            disabled
          ></el-input>
        </el-form-item>
        <el-form-item label="取消会议原因" prop="canceledreason">
          <el-input
            v-model="formRevoke.canceledreason"
            autocomplete="off"
          ></el-input>
        </el-form-item>
      </el-form>
    ...
    >
      <div style="margin-bottom: 10px; text-align: center">
        <el-tag type="danger">注意：不要提交重复的参会人员</el-tag>
      </div>
      <el-transfer
    ...
    >
      ...
      >
        ...
        <el-form-item label="会议开始签到时间" prop="signinstarttime">
          <el-date-picker
            v-model="formEdit.signinstarttime"
            type="datetime"
        ...
        <el-form-item label="会议结束签到时间" prop="signinendtime">
          <el-date-picker
            v-model="formEdit.signinendtime"
            type="datetime"
            placeholder="选择预定日期时间"
            value-format="yyyy-MM-dd HH:mm:ss"
          >
        ...
        <el-form-item label="预计参加会议的人数" prop="numberofparticipants">
          <el-input
            v-model="formEdit.numberofparticipants"
            autocomplete="off"
            disabled
          ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      this.request
        .get("/bookings/mybookings/" + this.employee.employeeid, {
          params: {
    ...
    DeleteEmp(employeeid, meetingid) {
      this.$confirm("确定要删除此员工参加此次会议吗？", "提示", {
        ...
        this.request
          .get("/addMeeting/delEmployee/" + meetingid + "/" + employeeid)
          .then((res) => {
            if (res.code === 200) {
              this.$message({
                showClose: true,
    ...
    revoke() {
      this.$refs["formRevoke"].validate((valid) => {
        if (valid) {
          this.request
            .post(
              "/bookings/cancelmeeting/" + this.meetingid,
              this.formRevoke.canceledreason
            )
            .then((res) => {
              console.log(res);
              if (res.code === 200) {
    ...
    addEmployee() {
      this.request
        .post("/addMeeting/addEmployee/" + this.meetingid, this.checked)
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
            this.$message({
    ...
    signature(meetingid, employeeid) {
      ...
      this.request
        .post("/addMeeting/signature", { meetingid, employeeid, location })
        .then((res) => {
          if (res.code === 200) {
            this.$message({
              showClose: true,
